...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4 mt-3">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Profilo') }}</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 fw-bold text-primary">{{ __('Informazioni profilo') }}</h6>
        </div>
        <div class="card-body">
            ...
            @if ($mustVerifyEmail && ! auth()->user()->hasVerifiedEmail())
                ...
                @if ($status === 'verification-link-sent')
                    <p class="small text-success mt-2 mb-0">{{ __('Nuovo link inviato.') }}</p>
                @endif
            @endif
        </div>
    </div>
    ...
    <div class="card shadow mb-4 border-left-danger">
        <div class="card-body">
            <h6 class="text-danger fw-bold mb-2">{{ __('Elimina account') }}</h6>
            <p class="small text-muted mb-3">{{ __('Questa azione è irreversibile.') }}</p>
